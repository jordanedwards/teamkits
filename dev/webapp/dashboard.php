    ...
    <div class="container">
		<div class="row">
				...
				<div class="widget widget-table action-table">
					...
					<div class="widget-content">
						<table class="table table-striped table-bordered">
							...
							<?php 
			$dm = new DataManager();
			$query="SELECT club_name, name AS customer_name, order_date_created, order_total, order_id FROM orders
			LEFT JOIN club ON orders.order_club_id = club.club_id
			LEFT JOIN customer ON orders.order_customer_id = customer.id
			...
			while ($row = mysqli_fetch_array($result))
				{
					$name = '';
					if ($row['club_name'] != ""){$name = $row['club_name'];} else {$name = $row['customer_name'];}
					echo '
					<tr>
