...
//Payment submitted, create order.
	...
	try {
		...
		if ($payment_success):
			...
			$order->set_date_submitted(date("Y-m-d g:i:s",time()));
			$order->save();
			...
			foreach($item_array as $name => $val){
				if ($val['jerseyRecord_id'] > 0){
					...
					if ($jersey_record->get_id() > 0){
						...
						$jersey_record->set_child_order_id($order->get_id());
						$jersey_record->set_price($val['price']);
						$jersey_record->save();
						
						$orderitem->get_by_id($jersey_record->get_orderitem_id());
						$parent_order = $orderitem->get_order_id();
						$jersey_payments = $jersey_payments + $val['price'];
					}
				} else {
			...
			if ($_POST['shippingcharges_hidden'] > 0){
				$orderitem->clear();
				$orderitem->set_order_id($order->get_id());
			...
			// Will need to expand this at some point to allow for multiple jerseys on DIFFERENT parent orders, but on the same child order.
			if ($parent_order > 0){
				$new_payment = new Payment();
				$new_payment->set_order_id($parent_order);
				$new_payment->set_transaction_number($token);
				$new_payment->set_amount($jersey_payments);
			...
			//Add payment record (to child order,if part of group order, or just regular order otherwise)
			$new_payment = new Payment();
			...
			if ($_POST['email'] != ""){
				...
				" . $cart->show_items() . "<br>
				<a href='https://teamkits.net/webapp/orders_edit.php?id=" . $order->get_id() . "'><em>Go to order</em></a>
				";
				$mail->Body = $body;
			...
			<div class="col-md-4">	
				...
				<form action="" method="POST" style="display: inline-block;">
					...
					<?php 
						...
						$dd->display();
					?>
					<input name="postal" id="postal" class="form-control inline" placeholder="Postal/Zip Code" required value="<?php echo $session->getData("postal"); ?>"><br>
