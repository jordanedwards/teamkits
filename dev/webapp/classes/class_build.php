<?php 
 class Build extends SessionManager {
...
public function save() {
		...
		try{
			...
			if($this->get_id() == 0) {
				...
				$strSQL = "INSERT INTO build (id, club_id, title, note, is_active, date_created, last_updated, last_updated_user) 
        VALUES (
	...
	public function delete() {
		try{
			$dm = new DataManager();
			$strSQL = "UPDATE build SET is_active='N' WHERE id=" . $this->id;
			$result = $dm->updateRecords($strSQL);
			return $result;
		}
		catch(Exception $e) {
	...
	public function get_by_id($id) {
		try{
			$status = false;
			$dm = new DataManager();
			$strSQL = "SELECT * FROM build WHERE id=" . $id;
