<?php 
 class BuildItem extends SessionManager {
	...
	function __construct() {
		...
		public function get_build_id() { return $this->build_id;}
		public function set_build_id($value) {$this->build_id=$value;}
...
public function save() {
		...
		try{
			...
			if($this->get_id() == 0) {
				...
				$strSQL = "INSERT INTO buildItem (id, build_id, item_id, is_active, date_created, last_updated, last_updated_user) 
        VALUES (
				'".mysqli_real_escape_string($dm->connection, $this->get_id())."',
				'".mysqli_real_escape_string($dm->connection, $this->get_build_id())."',
	...
	public function delete() {
		try{
			$dm = new DataManager();
			$strSQL = "UPDATE buildItem SET is_active='N' WHERE id=" . $this->id;
			$result = $dm->updateRecords($strSQL);
			return $result;
		}
		catch(Exception $e) {
	...
	public function get_by_id($id) {
		try{
			$status = false;
			$dm = new DataManager();
			$strSQL = "SELECT * FROM buildItem WHERE id=" . $id;
  	...
  	public function load_from_post($array){
		// Pass $_POST to this function, and if the post vars match the object methods (they should, using this program), then it will populate the object
  		foreach ($array as $key => $val){
			if(property_exists('buildItem',$key)):
