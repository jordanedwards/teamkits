    ...
    <div class="container">
      <div class="row">
        ...
        <h1>View Order #<?php echo $order->get_id()?> <i class="fa fa-print fa-right no_print"></i></h1>
			...
			<p>Click on the shopping cart icon next to the item to add to your cart.</p>
...
<table class="admin_table">
			<thead>
			<tr><th colspan="6">Order Items: </th></tr>
			<tr><th ></th><th>Item:</th><th>#</th><th>Name</th><th>Size</th><th>Price</th></tr>		
			</thead>
		 ...
		 <?php 
			...
			ORDER BY orderitem.orderitem_item_number
			";						
				...
				while($row = mysqli_fetch_assoc($result)):
					...
					// Don't show shipping charge
						...
						if ($row['status'] == "paid"){
							...
							echo '<tr><td><a class="add-item no_print" data-orderitem-id="' . $row['orderitem_id'] .'" data-jerseyrecord-id="' .$row['jerseyRecordId']. '"><i class="fa fa-cart-plus fa-lg"></i></a></td>';		
						}
						echo '<td><a class="imagePreview" data-item-id="' . $row['orderitem_item_number'] . '">' . $row['item_name'] . '</a></td><td>' . $row['number'] . '</td><td>' . $row['name'] . '</td><td>' . $row['orderitem_size'] . '</td><td style="white-space:normal; text-align:right;">$'.sprintf("%.2f",$row['orderitem_price']+$charge_per_item) .'</td></tr>';
					}
				endwhile;						
			endif;
