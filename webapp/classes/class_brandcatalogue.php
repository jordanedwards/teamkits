<?php 
 class Brandcatalogue extends SessionManager {
	...
	public function set_brand_id($value) {$this->brand_id=$value;}

	public function get_brand_name() { 
		// Return brand name if already set (i.e., this is an existing record, so it was loaded in the get_by_id()
		if (isset($this->brand_name)){
			return $this->brand_name;
		}else {
			// This is a new record, so we have to grab the brand name manually:
		 	$dm = new DataManager(); 
			$strSQL = "SELECT brand_name 
			FROM brand 
			WHERE brand_id=" . $this->get_brand_id();
	...
	public function set_brand_name($value) {$this->brand_name=$value;}
	...
	public function save() {
		...
		try{
			...
			if($this->get_id() == 0) {
				...
				$strSQL = "INSERT INTO brandcatalogue (id, brand_id, url, image, title, description, is_active, date_created, last_updated, last_updated_user) 
        VALUES (
				'".mysqli_real_escape_string($dm->connection, $this->get_id())."',
				'".mysqli_real_escape_string($dm->connection, $this->get_brand_id())."',
				...
				$strSQL = "UPDATE brandcatalogue SET 
					brand_id='".mysqli_real_escape_string($dm->connection, $this->get_brand_id())."',						 
					url='".mysqli_real_escape_string($dm->connection, $this->get_url())."',						 
					image='".mysqli_real_escape_string($dm->connection, $this->get_image())."',						 
					title='".mysqli_real_escape_string($dm->connection, $this->get_title())."',						 
					description='".mysqli_real_escape_string($dm->connection, $this->get_description())."',						 
	...
	public function delete() {
		try{
			$dm = new DataManager();
			$strSQL = "UPDATE brandcatalogue SET is_active='N' WHERE id=" . $this->id;
			$result = $dm->updateRecords($strSQL);
			return $result;
		}
		catch(Exception $e) {
	...
	public function get_by_id($id) {
		try{
			if ($id > 0){
				...
				LEFT JOIN brand ON brandcatalogue.brand_id = brand.brand_id
				WHERE id=" . $id;
  	...
  	public function load_from_post($array){
		// Pass $_POST to this function, and if the post vars match the object methods (they should, using this program), then it will populate the object
  		foreach ($array as $key => $val){
			if(property_exists('brandcatalogue',$key)):
	...
	// loads the object data from a mysql assoc array
  	private function load($row){
		$this->set_id($row["id"]);
		$this->set_brand_id($row["brand_id"]);
		$this->set_brand_name($row["brand_name"]);		
