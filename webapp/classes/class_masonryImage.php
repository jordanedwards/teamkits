<?php 
 class MasonryImage extends SessionManager {
	...
	public function save() {
		...
		try{
			...
			if($this->get_id() == 0) {
				...
				$strSQL = "INSERT INTO masonryImage (id, url, is_active, date_created, last_updated, last_updated_user) 
        VALUES (
				...
				$strSQL = "UPDATE masonryImage SET 
	...
	public function delete() {
		try{
			$dm = new DataManager();
			$strSQL = "UPDATE masonryImage SET is_active='N' WHERE id=" . $this->id;
			$result = $dm->updateRecords($strSQL);
			return $result;
		}
		catch(Exception $e) {
	...
	public function get_by_id($id) {
		try{
			if ($id > 0){
				$status = false;
				$dm = new DataManager();
				$strSQL = "SELECT * FROM masonryImage WHERE id=" . $id;
  	...
  	public function load_from_post($array){
		// Pass $_POST to this function, and if the post vars match the object methods (they should, using this program), then it will populate the object
  		foreach ($array as $key => $val){
			if(property_exists('masonryImage',$key)):
