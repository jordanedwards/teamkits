...
public function __toString(){
	...
	public function login() {
		...
		try{
			$dm = new DataManager();
			$strSQL = "SELECT user_id, user_role FROM user WHERE user_email='" . $this->get_email() . "' AND user_password='" . $this->get_password() . "' AND is_active='Y'";
...
public function save() {
		try{
			...
			if($this->get_id() == 0) {
				...
				$strSQL = "INSERT INTO user (user_id, user_first_name, user_last_name, user_email, user_tel, user_password, user_login, user_role, user_date_created, user_last_updated, user_last_updated_user, is_active) 
        VALUES (
